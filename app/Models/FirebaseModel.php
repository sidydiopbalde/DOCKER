...
{
    $encodedCredentials = env("FIREBASE_CREDENTIALS");
    $databaseUrl = env('FIREBASE_DATABASE_URL');
    $credentials = json_decode(base64_decode($encodedCredentials), true);

    // if (is_null($encodedCredentials) || is_null($databaseUrl)) {
    //     throw new \Exception('Les variables d\'environnement FIREBASE_CREDENTIALS ou FIREBASE_DATABASE_URL ne sont pas définies.');
    // }
    ...
    if (json_last_error() !== JSON_ERROR_NONE) {
        throw new \Exception('Erreur lors du décodage des credentials Firebase.');
    }
